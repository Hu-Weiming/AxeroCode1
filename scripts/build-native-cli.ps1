...
if ((-not (Get-Command cl -ErrorAction SilentlyContinue)) -and (-not (Test-Path $vcvars))) {
    Write-Error "Microsoft C/C++ build tools were not found. Native CLI build requires the Windows linker toolchain."
    exit 1
}

if (-not (Get-Command cl -ErrorAction SilentlyContinue)) {
    cmd /c "set ""TEMP=$nativeTemp"" && set ""TMP=$nativeTemp"" && set ""NATIVE_IMAGE_USER_HOME=$nativeUserHome"" && call `"$vcvars`" >nul && call `"$mvnCmd`" -q -pl axercode-cli -am -Pnative package"
    exit $LASTEXITCODE
}

& $mvnCmd -q -pl axercode-cli -am -Pnative package
exit $LASTEXITCODE
